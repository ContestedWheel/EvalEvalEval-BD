...
try:
    ...
    await ctx.send(
        f"Attempt to change {settings.collectible_name}'s spawn art failed.\n"
        f"{erm}\n{e}\n{erm}"
    )
    return

ball.wild_card = "/" + str(image_path)

await ball.save()

await ctx.send(f"Successfully changed {ball.country}'s spawn art.")